...
def generate_signals(df_5m, df_30m, df_1h, df_4h,return_data):
    ...
    adx = df_4h.iloc[-1]['adx']
    cmf = df_4h.iloc[-1]['cmf']
    supertrend = df_4h.iloc[-1]['supertrend']
    ema_50 = df_4h.iloc[-1]['ema_50_signal']
    ema_200 = df_4h.iloc[-1]['ema_200_signal']
    macd = df_4h.iloc[-1]['macd_signal']
    atr = df_4h.iloc[-1]['atr']
    close_4h = df_4h.iloc[-1]['close']

    if close_4h>supertrend:
        supertrend = 'Bullish'
    else:
        supertrend = 'Bearish'
    ...
    print('----------------------------------------------------------------')    
    insert_4h_into_mongo(mongo_id,bias_4h_zone,pearson_4h_value,adx,cmf,supertrend,ema_50,ema_200,macd,atr)
    print('----------------------------------------------------------------')

def get_data_4h(return_data = False):
    initialize_mt5()
    ...
    if int(current_hour) % 4 == 0:
        ...
        generate_signals(df_5m, df_30m, df_1h, df_4h,return_data)

    
    df_5m.rename(columns={'rsi':'sma',},inplace=True)
